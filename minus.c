...
void Input(struct complex buf[2]){
    if(buf[0].Im || buf[1].Im)
        printf("Minus process: input %lf+i%lf, %lf+i%lf\n", buf[0].Re, buf[0].Im, buf[1].Re, buf[1].Im);
    else
        printf("Minus process: input %lf, %lf\n", buf[0].Re, buf[1].Re);
}

void Output(struct complex res){
    if(res.Im)
        printf("Minus process: output %lf+i%lf\n", res.Re, res.Im);
    else
        printf("Minus process: output %lf\n", res.Re);
}
...
void sigint_handler(int sig) {
        ...
        res.Re = buf[0].Re-buf[1].Re;
        res.Im = buf[0].Im-buf[1].Im;
